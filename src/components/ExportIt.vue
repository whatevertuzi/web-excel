<template>
  ...
  <el-button v-on:click="exports">导出日报</el-button>
  <el-button v-on:click="exportRecord">导出呼损回访</el-button>
      ...
      if (this.value2[0].toString() != '' && this.value2[1].toString() != '') {
        ...
        axios
            ...
            .then(function (response) {
              ...
              downloadElement.download = 'IT' + String(begintime).substring(0, 10) + '.xlsx'; //下载后文件名-->
      ...
      if (this.value2[0].toString() != '' && this.value2[1].toString() != '') {
        ...
        axios
            ...
            .then(function (response) {
              ...
              var href = window.URL.createObjectURL(blob); //创建下载的链接-->
              downloadElement.href = href;
              downloadElement.download = 'IT呼损回访' + String(begintime).substring(0, 10) + '.xlsx'; //下载后文件名-->
              document.body.appendChild(downloadElement);
              downloadElement.click(); //点击下载-->
              document.body.removeChild(downloadElement); //下载完成移除元素-->
              window.URL.revokeObjectURL(href); //释放掉blob对象-->
        ...
        if (new Date - this.sendTime > 20 * 1000) {
          ...
          axios.post(
              "http://10.3.28.20:8889/sendemail", {
                ...
                domain: "helpdesk.greentree.cn"
              }
          ).then(res => {
            ElMessage({
              message: res.data.msg + ":邮件已发送！",
              type: 'success',
            })
          }).catch(function (error) {
            ElMessage({
